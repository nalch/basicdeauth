...
server_settings = {}
for settings_file in glob.glob(os.path.join(BASE_DIR, 'basicdeauth', 'settings_pipeline', '*.json')):
...
try:
    tmp = server_settings.copy()
    tmp.update(json.load(open(os.path.join(BASE_DIR, 'basicdeauth', 'settings_pipeline', 'stages', STAGE+'.json'))))
    server_settings = tmp
except IOError:
    pass

VAR_PATH = server_settings.get('VAR_PATH', os.path.join(os.sep, 'var', 'local', 'basicdeauth'))
CONF_PATH = server_settings.get('CONF_PATH', os.path.join(os.sep, 'etc', 'local', 'basicdeauth', 'conf'))
...
ROOT_URLCONF = 'basicdeauth.urls'

WSGI_APPLICATION = 'basicdeauth.wsgi.application'
...
LOGGING = {
    ...
    'handlers': {
        'file': {
            ...
            'filename': LOG_PATH + '/basicdeauth.log',
        },
    },
    'loggers': {
        'django.request': {
